...
![ClassLoader加载过程](/assets/01.java提升计划/02.jvm/02/ClassLoader加载过程.png)
...
它的加载过程是怎么样的呢?  
1. 执行 main() 方法（静态方法）就需要先加载main方法所在类 HelloLoader
2. 加载成功，则进行链接、初始化等操作。完成后调用 HelloLoader 类中的静态方法 main
3. 加载失败则抛出异常  
![加载类顺序](/assets/01.java提升计划/02.jvm/02/加载类顺序.png)

完整的流程图如下所示：  
![类加载完整流程](/assets/01.java提升计划/02.jvm/02/类加载完整流程.png)
...
1. 验证(Verify)  
    ...
    3. 举例:使用 BinaryViewer软件查看字节码文件，其开头均为 CAFE BABE ，如果出现不合法的字节码文件，那么将会验证不通过。
...
2. 准备(Prepare)  
    ...
    4. 举例:代码：变量a在准备阶段会赋初始值，但不是1，而是0，在初始化阶段会被赋值为 1  
    ```java
...
3. 解析(Resolve)  
    ...
    5. 符号引用:反编译 class 文件后可以查看符号引用，下面带`#`的就是符号引用    
    ![符号引用](/assets/01.java提升计划/02.jvm/02/符号引用.png)
...
4. clinit()不同于类的构造器。（关联：构造器是虚拟机视角下的init()）
...
![加载器分类](/assets/01.java提升计划/02.jvm/02/加载器分类.png)   
4. 我们尝试获取引导类加载器，获取到的值为 null ，这并不代表引导类加载器不存在，因为引导类加载器右 C/C++ 语言，我们获取不到  
5. 两次获取系统类加载器的值都相同：sun.misc.Launcher$AppClassLoader@18b4aac2 ，这说明系统类加载器是全局唯一的  
...
#### 系统类加载器(也称为系统类加载器，AppClassLoader)
...
![ClassLoader类](/assets/01.java提升计划/02.jvm/02/ClassLoader类.png)

#### 获取ClassLoader途径
![获取ClassLoader途径](/assets/01.java提升计划/02.jvm/02/获取ClassLoader途径.png)  
```java
...
![双亲委派机制原理](/assets/01.java提升计划/02.jvm/02/双亲委派机制原理.png)
...
```   
![双亲委派机制代码示例错误](/assets/01.java提升计划/02.jvm/02/双亲委派机制代码示例错误.png)  
...
![举例3](/assets/01.java提升计划/02.jvm/02/举例3.png)
